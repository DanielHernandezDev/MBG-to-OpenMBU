...
struct ConnectData
{
   ...
   float3 outLightVecSec  : TEXCOORD4;
...
)
{
   ...
   OUT.outLightVecSec.xyz = normalize(lightPosSec.xyz - IN.position.xyz);
   OUT.dlightCoordSec.w = saturate(dot(OUT.outLightVecSec, IN.N) * 4.0);
   OUT.outLightVec.xyz = mul(objToTangentSpace, OUT.outLightVec);
   OUT.outLightVecSec.xyz = mul(objToTangentSpace, OUT.outLightVecSec);
